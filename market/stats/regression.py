...
def fit(y: pd.Series, X: pd.Series | pd.DataFrame):
    """OLS of y on X (with constant), using common non-missing dates."""
    data = pd.concat([y, X], axis=1).dropna()
    return sm.OLS(data.iloc[:, 0], sm.add_constant(data.iloc[:, 1:])).fit()
...
def capm(r: pd.Series, bench: pd.Series, periods: int | None = None) -> pd.Series:
